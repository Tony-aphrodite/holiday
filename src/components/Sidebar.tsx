...
export default function Sidebar() {
  ...
  function isActive(item: NavItem) {
    ...
    if (item.route.name === 'customers') {
      return route.name === 'customers' || route.name === 'customer';
    }
    if (item.route.name === 'people') {
      return route.name === 'people' || route.name === 'chat';
    }
    return false;
  }
    ...
    <aside className="hidden md:flex w-60 shrink-0 flex-col border-r border-border bg-bg-soft/60 backdrop-blur">
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        ...
        {nav.map((item) => {
          ...
          const badge =
            item.badgeKey === 'customers'
              ? customers.length
              : item.badgeKey === 'people'
                ? people.length
                : undefined;
          return (
            <button
